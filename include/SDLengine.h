...
void LTexture::render(SDL_Renderer *&gRenderer, int x, int y,int lx=0,int ly=0, SDL_Rect* clip, double angle, SDL_Point* center, SDL_RendererFlip flip )
{
...
bool initSDL(const char* title, int height, int width ,SDL_Window* &gWindow,SDL_Renderer* &gRenderer)
{
	//Initialization flag
	bool success = true;
    ...
    Mix_OpenAudio(49000,MIX_DEFAULT_FORMAT,2,16384);
		...
		//Create window
		gWindow = SDL_CreateWindow( title, SDL_WINDOWPOS_CENTERED,SDL_WINDOWPOS_CENTERED ,height, width, SDL_WINDOW_SHOWN );
